if(undefined === L.Map.prototype.eachLayer) {
  L.Map.prototype.eachLayer = function (a,b){for(var c in this._layers)a.call(b,this._layers[c]);return this;}
} // old leaflets

function leafletImage(map, callback) {
    ...
    if (map._pathRoot) {
      // TODO: this breaks for me, blame old version of leaflet?
        ...
        tiles.forEach(function(tilePoint) {
            ...
            tileQueue.awaitAll(function(err, data) {
                data.forEach(function(d) {
                    if(d.img.height !== 0) ctx.drawImage(d.img, Math.floor(d.pos.x), Math.floor(d.pos.y),
                        d.size, d.size);
                });
                callback(null, {
                    canvas: canvas
